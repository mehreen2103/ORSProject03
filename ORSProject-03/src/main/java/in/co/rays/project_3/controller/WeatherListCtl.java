...
@WebServlet(name = "WeatherListCtl", urlPatterns = { "/ctl/WeatherListCtl" })

public class WeatherListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(WeatherListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("WeatherListCtl doGet Start");
		...
		log.debug("WeatherListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("WeatherListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					WeatherDTO deleteDto = new WeatherDTO();
			...
			dto = (WeatherDTO) populateDTO(request);
		...
		log.debug("WeatherListCtl doPost End");
	}

	@Override
	protected String getView() {

		return ORSView.WEATHER_LIST_VIEW;
	}
}