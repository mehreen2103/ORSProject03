...
@WebServlet(name = "UserProfileListCtl", urlPatterns = { "/ctl/UserProfileListCtl" })

public class UserProfileListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(UserProfileListCtl.class);
	...
	protected void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException, IOException {
		...
		log.debug("UserProfileListCtl doGet Start");
		...
		log.debug("UserProfileListCtl doGet End");
	}

	@Override
	protected void doPost(HttpServletRequest request,HttpServletResponse response)throws ServletException, IOException {

		log.debug("UserProfileListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op)|| OP_NEXT.equalsIgnoreCase(op)|| OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					UserProfileDTO deleteDto = new UserProfileDTO();
			...
			dto = (UserProfileDTO) populateDTO(request);
		...
		log.debug("UserProfileListCtl doPost End");
	}

	@Override
	protected String getView() {

		return ORSView.USERPROFILE_LIST_VIEW;
	}
