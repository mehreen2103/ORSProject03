...
@WebServlet(name = "SmartDeviceListCtl", urlPatterns = { "/ctl/SmartDeviceListCtl" })

public class SmartDeviceListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(SmartDeviceListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("SmartDeviceListCtl doGet Start");
		...
		log.debug("SmartDeviceListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("SmartDeviceListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					SmartDeviceDTO deleteDto = new SmartDeviceDTO();
			...
			dto = (SmartDeviceDTO) populateDTO(request);
		...
		log.debug("SmartDeviceListCtl doPost End");
	}

	@Override
	protected String getView() {

		return ORSView.SMARTDEVICE_LIST_VIEW;
	}
}