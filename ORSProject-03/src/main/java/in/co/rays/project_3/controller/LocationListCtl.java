...
@WebServlet(name = "LocationListCtl", urlPatterns = { "/ctl/LocationListCtl" })

public class LocationListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(LocationListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("LocationListCtl doGet Start");
		...
		log.debug("LocationListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("LocationListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					LocationDTO deleteDto = new LocationDTO();
			...
			dto = (LocationDTO) populateDTO(request);
		...
		log.debug("LocationListCtl doPost End");
	}

	@Override
	protected String getView() {

		return ORSView.LOCATION_LIST_VIEW;
	}
}