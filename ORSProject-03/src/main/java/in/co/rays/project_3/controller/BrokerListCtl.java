...
public class BrokerListCtl extends BaseCtl {
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		System.out.println("jswjcbuggco2ecouegh"+ request.getParameter("brokerName"));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("in doget meothod" );
		...
		System.out.println("iam here inpopoulate");
		// get the selected checkbox ids array for delete list
		BrokerModelInt model = ModelFactory.getInstance().getBrokeModel();
		try {
			list = model.search(dto, pageNo, pageSize);
			System.out.println("skbdiq--------------------------------"+list);

			/*
			 * ArrayList<UserDTO> a = (ArrayList<UserDTO>) list; 9)()(0 for (UserDTO udto1 :
			 * a) { System.out.println(udto1.getRoleId()); }
			 */
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					BrokerDTO deletebean = new BrokerDTO();
			...
			dto = (BrokerDTO) populateDTO(request);
