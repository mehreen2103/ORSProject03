...
@WebServlet(name = "PasswordListCtl", urlPatterns = { "/ctl/PasswordListCtl" })

public class PasswordListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;

	private static Logger log = Logger.getLogger(PasswordListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("PasswordListCtl doGet Start");
		...
		log.debug("PasswordListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("PasswordListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || OP_NEXT.equalsIgnoreCase(op) || OP_PREVIOUS.equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					...
					PasswordDTO deleteDto = new PasswordDTO();
			...
			dto = (PasswordDTO) populateDTO(request);
		...
		log.debug("PasswordListCtl doPost End");
	}

	@Override
	protected String getView() {

		return ORSView.PASSWORD_LIST_VIEW;
	}
}